  ...
  $.widget('ui.weekCalendar', (function() {
    ...
    return {
      options: {
        ...
        buttons_left:true,
        date_text:true,
        scroll_event_body: false,
        notOverlapInEdit: false,
        editOnlyNewEvent: false,//readonly debe estar en false
        notSameRoom: true,
        //******
        date: new Date(),
        timeFormat: null,
      ...
      _create: function() {
        ...
        if (this.options.editOnlyNewEvent) {
        ...
        if (this.options.notOverlapInEdit) {
            this.options.allowCalEventOverlap = false;
        }
      ...
      /*
        ...
        */
        ...
        /*
        * MI MODIFICACION
        * si se le pasa el parametro seleccion "true", entonces
        * 'selecciona' el evento poniendole la clase seleccionar_evento
        */

      updateEvent: function(calEvent,seleccion) {
          this._updateEventInCalendar(calEvent,seleccion);
      },
      ...
      check: function(elemento){
          ...
          //console.log(this._adjustOverlappingEvents(elemento));
      },
      ...
      _setupEventDelegation: function() {
        ...
        this.element.click(function(event) {
          ...
          if (options.allowEventDelete && $target.hasClass('wc-cal-event-delete')) {
              ...
              //console.log($calEvent.data('calEvent'));
        ...
        if (options.buttons) {
            ...
            calendarNavHtml += '<div class=\"ui-widget-header wc-toolbar\">';
              ...
              calendarNavHtml += '<div class=\"wc-nav\">';
                ...
                //(*1)calendarNavHtml += '<button class=\"wc-today\">' + options.buttonText.today + '</button>';
            ...
            if (options.buttons_left){
                /*
                 * mi modificacion
                 * saco el boton "today" tambien saque (*1) mas arriba
                ...
                $calendarContainer.find('.wc-nav .wc-prev')
                  ...
                  .click(function() {
                      //mi modificacion
                      //si es lunes no retroceder mas
                    if (self.element.find('.wc-day-1').html()!=options.longDays[1])
                ...
                $calendarContainer.find('.wc-nav .wc-next')
                  ...
                  .click(function() {
                      //mi modificacion
                      //si es domingo no avanzar mas
                    if (self.element.find('.wc-day-1').html()!=options.longDays[0])
                      self.element.weekCalendar('next');
                      return false;
                    });
            } else {
                $calendarContainer.find('.wc-nav').css('display','none');
            }
...
//              calendarHeaderHtml+=    "</div>";
  ...
  TODO    if total height for an hour is less than 11px, there is a display problem.
      ...
      _setupEventCreationForWeekDay: function($weekDay) {
          ...
          $weekDay.mousedown(function(event) {
            ...
            if ($newEvent.length) {
                ...
                $newEvent.remove();
                //mi modificacion
                //agregue el "id" aleatorio
                var newCalEvent = {start: eventDuration.start, end: eventDuration.end, title: options.newEventText, id: 'n'+Math.floor((Math.random() * 10000) + 1)};
      ...
      _updateDayColumnHeader: function($weekDayColumns) {
          ...
          //mi modificacion
          // mostrar la fecha solo si esta establedido button_left y date_text
          if (this.options.title && this.options.buttons_left && this.options.date_text) {
      ...
      _renderEvent: function(calEvent, $weekDay) {
          ...
          //mi modificacion
          //agregar scroll al cuerpo del evento
          //tambien se agrigan la opciones adicionales a mostrar
          //en el cuerpo del evento (hay que modificar tb la propiedad
          //_refreshEventDetails para mostrar el htm)
          if (options.scroll_event_body){
              //eventHtml += '<div id="body_scroll" style=\"overflow-y: auto; height:43px\">';
              eventHtml += '<div id="body_scroll">';
              eventHtml += '<div class=\"wc-title\">\n\</div>\n\
                            <div class=\"wc-body\">\n\</div>\n\
                            <div class=\"wc-disponibilidad\"></div>\n\
                            <div class=\"wc-salon\"></div>';  
              eventHtml += '</div></div>';
          } else {
            ...
            $.each(groupsList, function() {
                ...
                $.each(curGroups, function(groupIndex) {
                  ...
                  $.each(curGroup, function() {
                      ...
                      //mi modificacion: para que tenga scroll el cuerpo del evento si no alcanza a mostrar el texto
                      var overflow_body = $(this).outerHeight() - $(this).find(".wc-time").outerHeight();
                      $(this).find("#body_scroll").css({"overflow-y": "auto", "height":overflow_body});
                  });
                });
            });
          }
      ...
      _checkOverlapsSameRoom: function($weekDay){
          ...
          $.each(sortedEvents, function() {
            ...
            for (var groupIndex = 0; groupIndex < curGroups.length; groupIndex++) {
                if (curGroups[groupIndex].length > 0) {
                    console.log(curGroups[groupIndex][curGroups[groupIndex].length - 1].data('calEvent').id_salon);
                    console.log('Salon Actual: '+$curEvent.data('calEvent').id_salon);
                    if (curGroups[groupIndex][curGroups[groupIndex].length - 1].data('calEvent').id_salon==$curEvent.data('calEvent').id_salon){
                        console.log('puto');
                        return false;
                    }
            ...
            curSalon = $curEvent.data('calEvent').id_salon;
         });
          return true;          
      },
      /*
      ...
      /*
        ...
        */
      _updateEventInCalendar: function(calEvent,seleccion) {
          var self = this;
          ...
          if ($weekDays) {
            $weekDays.each(function(index, weekDay) {
              ...
              /*
               * MI MODIFICACION
               * si se le pasa el parametro seleccionado de calEvent "true", entonces
               * 'selecciona' el evento poniendole la clase seleccionar_evento
               */
            if (calEvent.seleccionado)
                $calEvent.addClass('seleccionar_evento');
            });
          }
      },
      ...
      _adjustForEventCollisions: function($weekDay, $calEvent, newCalEvent, oldCalEvent, maintainEventDuration) {
          var options = this.options;
//mi modificacion
//agrego la variable notSameRoom para no permitir el overlap de eventos en el mismo salon
//          if (options.allowCalEventOverlap && !options.notSameRoom) {

console.log(this._checkOverlapsSameRoom($weekDay));
          if (options.allowCalEventOverlap) {
              //aca tengo que preguntar si existe overlap de dos eventos en el mismo salon.
              //si no existe hago un return, si no sigo con la funcion
            return;
          }
	  ...
	  if (!this.options.readonly &&
		 ...
		 this.options.deletable(calEvent,$calEvent)) {
          ...
          $calEvent.find('.wc-disponibilidad').html(calEvent.disponibilidad);
          $calEvent.find('.wc-salon').html(calEvent.salon);
