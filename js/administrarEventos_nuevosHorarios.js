...
function mostrar_horarios(){
    if ($('table#tabla_salones > tbody > tr.row_selected').length && tabla_eventos.$('tr.row_selected').length){
        ...
        id_profesor_seleccionado = $('table#tabla_eventos > tbody > tr.row_selected > td:nth-child(2)').prop('id');
        ...
        $.ajax({
            ...
            data: {id_salon: id_salon_seleccionado, id_evento: id_evento_seleccionado, id_profesor: id_profesor_seleccionado},
            dataType: 'json',
...
function guardarCambiosNuevosHorarios(){
    //console.log(jQuery.isEmptyObject(horarios_borrados));
   
    $.ajax({
       type: 'POST',
       url: url_guardarCambiosHorarios,
       data: {cambiados: (jQuery.isEmptyObject(horarios_cambiados))? null:horarios_cambiados, borrados: (jQuery.isEmptyObject(horarios_borrados))? null:horarios_borrados, nuevos: (jQuery.isEmptyObject(horarios_nuevos))? null:horarios_nuevos},
       //dataType: 'json',
        beforeSend:function(){
            $('#btn_guardar_cambios').html("<span id='span_enviar' class='fa fa-spinner fa-spin'></span>  Procesando......").removeClass('btn-primary').addClass('btn-info');
            $('#btn_guardar_cambios').attr('disabled', 'disabled').addClass('disabled');
        },
       success: function(resultado){
            var url2 = Yii.app.createUrl('administrarEventos/nuevosHorarios',{guardado: true});
           window.location = url2;
           //$('#nuevosHorarios').html(resultado);
       },
       error: function(error){
           alert('error');
            var url2 = Yii.app.createUrl('administrarEventos/nuevosHorarios');
            window.location = url2;
       }
    });
...
function calendario_nuevos_horarios(datos){
   ...
   $calendar2.weekCalendar({
      ...
      eventNew : function(calEvent, $event) {
            ...
            calEvent['id_evento'] = id_evento_seleccionado;
            calEvent['borrable'] = true,
