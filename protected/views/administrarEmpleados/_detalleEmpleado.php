<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var mensaje;
        if (estado==1)
            mensaje = "El empleado ha sido deshabilitado</br>manualmente por el administrador";
        if (estado!=0)
            $('#tooltip').tooltip({'placement':'right', 'trigger' : 'hover', title: mensaje, html: true});
        
    });
    
    function guardarDatosFormModificarEmpleado(){
        var data=$("#empleado-form").serialize();
        var url = '<?php echo Yii::app()->createAbsoluteUrl("administrarEmpleados/detalles",array('id'=>$model->id_empleado)); ?>';
        $.ajax({
            type: 'POST',
            url: url,
            data:data,
            ...
            success:function(result){
                if (result=='')
                    ...
                    console.log(result);
                    restaurarDatosFormModificarEmpleado();
    ...
    <div class="span12">
        <div class="row-fluid row_paddin_boton">
            ...
            <div class="span3">
                    ...
                    <h4>Tipo: </h4><span class="label label-default"><h4><?php echo ($model->tipo==0)? 'Profesor':'Otro';?></h4></span>                  
            </div>
...
?>
    ...
    <div class="modal-header">
        <a class="close" data-dismiss="modal" onclick="restaurarDatosFormModificarEmpleado();">&times;</a>
        <h4>Modificar datos</h4>
    </div>
 
    <div class="modal-body">
        <?php $this->renderPartial('_formModificarEmpleado',array('model'=>$model));?>
    </div>
    ...
    <div class="modal-footer">
        <?php $this->widget(
            ...
            array(
                ...
                'label' => 'Guardar',
                'url' => '#',
                'htmlOptions' => array('onclick'=>'js:guardarDatosFormModificarEmpleado();'),
            )
        ); ?>
        <?php $this->widget(
            'bootstrap.widgets.TbButton',
            array(
                ...
                'label' => 'Cancelar',
                'url' => '#',
                'htmlOptions' => array('data-dismiss' => 'modal', 'onclick'=>'js:restaurarDatosFormModificarEmpleado();'),
            )
        ); ?>
    </div>
<?php $this->endWidget(); ?>