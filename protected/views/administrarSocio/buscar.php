...
<script>
    ...
    function eliminar(idSocio, nombre, apellido){
        console.log($("#myModal4"));
        url = Yii.app.createUrl('administrarSocio/eliminar',{id_socio: idSocio});
...
$this->widget('application.components.GGridView', array(
     ...
     */
        ...
        'footer_count'=>true,
        'footer_count_text'=>'Socios encontrados:',
    ...
    //ESTA ECHO EN CGridColumn CON COMENTARIOS EN LA LINEA 114
		...
		array(
			'name'=>'estado',
			'value'=>'($data->estado==0)? "Habilitado":"No habilitado"',
			'filter'=>CHtml::dropDownList('Socio[estado]', $model->estado, 
                                    array(''=>'Todos', '0'=>'Habilitado', '1'=>'No habilitado',)
                                ),
                        //'htmlOptions'=>array('class'=>'label label-($data->estado==0)? "success":"danger"'),
		),
                ...
                array(
                    ...
                    'value'=>function($data){
                        $btn_detalles = CHtml::link('<i class="icon-search"></i>',
                                            array('administrarSocio/detalles','id'=>$data->id_socio), 
                        ...
                        $btn_eliminar = CHtml::htmlButton('<i class="icon-remove"></i>',
                                            array('onclick'=>'eliminar('.$data->id_socio.',"'.$data->nombre.'","'.$data->apellido.'");','class'=>'pull-right btn btn-danger btn-small')
                                        );
...
?>
    ...
    <div class="modal-body">
        <p>Esto borrara al socio <span id="nombreCompleto"> y todos sus registros asociados.</span></p>
